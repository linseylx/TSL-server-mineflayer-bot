...
```json
{
  ...
  "mainAdminPort": 28474,
  "transportToken": "云端 transport_token.txt 内容"
}
```

云端首次启动会自动生成 `transport_token.txt`。本地运输 Bot 没有公网 IP 也能工作，但必须能访问云端后台端口，并且每个 `/api/bot/*` 和本地更新下载请求都要携带这个 token。

如果后台前面有 HTTPS 反向代理，可以改用完整地址：

```json
{
  "mainBaseUrl": "https://你的域名",
  "transportToken": "云端 transport_token.txt 内容"
}
```
