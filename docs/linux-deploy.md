...
本地运输 Bot 更新包如果涉及 `transport_bot.js` 或 `auto_updater.js`，必须确保本地配置中已有 `transportToken`。云端 `/api/bot/*`、`/api/update/local/manifest`、`/api/update/local/download` 会校验 `X-Transport-Token`。
