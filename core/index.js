...
let mainChatQueue = Promise.resolve()
let mainLastChatSentAt = 0
const mainRecentChatCommands = new Map()
...
let foodRecoveryInProgress = false
...
function safeChat(message, options = {}) {
  const command = String(message || '').trim()
  if (!command) return Promise.resolve(false)
  const dedupeMs = Number.isFinite(Number(options.dedupeMs)) ? Number(options.dedupeMs) : 1000
  const previousAt = mainRecentChatCommands.get(command) || 0
  if (dedupeMs > 0 && Date.now() - previousAt < dedupeMs) {
    log('聊天限速', '跳过重复命令: ' + command, 'yellow')
    return Promise.resolve(false)
  }
  const minGapMs = Number.isFinite(Number(options.minGapMs)) ? Number(options.minGapMs) : Number(process.env.MAINBOT_CHAT_MIN_INTERVAL_MS || 1600)
  mainChatQueue = mainChatQueue.then(async () => {
    if (!state.bot || typeof state.bot.chat !== 'function') {
      log('聊天失败', command, 'yellow')
      return false
    }
    const waitMs = Math.max(0, minGapMs - (Date.now() - mainLastChatSentAt))
    if (waitMs > 0) await sleep(waitMs)
    state.bot.chat(command)
    mainLastChatSentAt = Date.now()
    mainRecentChatCommands.set(command, mainLastChatSentAt)
    for (const [key, at] of mainRecentChatCommands) {
      if (Date.now() - at > 60000) mainRecentChatCommands.delete(key)
    }
    return true
  }).catch(err => {
...
function handleWelcomeBindCommand(sender, message) {
  ...
  const match = text.match(/^!?\s*欢迎\s+([A-Za-z0-9_]{1,16})$/)
...
async function startPrivateChestSequence() {
  ...
  try {
    ...
    await safeChat('/phome anyun_安芸工业区', { minGapMs: 1800, dedupeMs: 2000 })
...
async function openChestAtPosition(x, y, z, options = {}) {
  ...
  try {
    ...
    if (options.skipMove && currentDist > maxOpenDistance) {
      appendToChatFile(`距离箱子过远且本次禁止自动移动，跳过: (${x}, ${y}, ${z}) distance=${currentDist.toFixed(1)}`)
      return false
    }
    if (!options.skipMove && currentDist > maxOpenDistance) {
      const standX = x
...
async function moveToPosition(x, y, z, options = {}) {
  ...
  return new Promise((resolve) => {
    try {
      ...
      const minTimeoutMs = Number.isFinite(Number(options.minTimeoutMs)) ? Number(options.minTimeoutMs) : 8000
      const maxTimeoutMs = Number.isFinite(Number(options.maxTimeoutMs)) ? Number(options.maxTimeoutMs) : 60000
      const computedTimeoutMs = Math.min(Math.max(distance * 700, minTimeoutMs), maxTimeoutMs)
      ...
      const timeoutId = setTimeout(() => {
        ...
        const finalDistance = livePos.distanceTo(targetPos)
        appendToChatFile(`移动超时: 目标 (${x}, ${y}, ${z})，当前位置: (${Math.floor(livePos.x)}, ${Math.floor(livePos.y)}, ${Math.floor(livePos.z)})，距离 ${finalDistance.toFixed(1)} 格`)
        finish(finalDistance <= goalRange + 0.75)
      }, timeoutMs)
      
      const onGoalReached = () => {
        bot.pathfinder.setGoal(null)
        const finalPos = bot.entity.position
        const finalDistance = finalPos.distanceTo(targetPos)
        appendToChatFile(`已到达移动目标附近: (${Math.floor(finalPos.x)}, ${Math.floor(finalPos.y)}, ${Math.floor(finalPos.z)})，距离 ${finalDistance.toFixed(1)} 格`)
        finish(finalDistance <= goalRange + 0.75)
      }
      ...
      const onPathReset = (reason) => {
        const livePos = bot.entity.position
        const finalDistance = livePos.distanceTo(targetPos)
        appendToChatFile(`寻路重算: ${reason || 'unknown'}，当前位置: (${Math.floor(livePos.x)}, ${Math.floor(livePos.y)}, ${Math.floor(livePos.z)})，距目标 ${finalDistance.toFixed(1)} 格`)
      }
      ...
      const onPathUpdate = (result) => {
        ...
        const finalDistance = livePos.distanceTo(targetPos)
        bot.pathfinder.setGoal(null)
        appendToChatFile(`无法找到路径: 目标 (${x}, ${y}, ${z})，当前位置距离 ${finalDistance.toFixed(1)} 格`)
        finish(finalDistance <= goalRange + 0.75)
      }
      ...
      appendToChatFile(`移动异常: ${e.message}`)
      resolve(false)
    }
  })
}
...
function handleBindVerification(username, message) {
  ...
  if (isBotReady() && state.bot.players[username]) {
    ...
    log('绑定', `玩家 ${username} 的 UUID: ${playerUuid}, 城镇: ${playerTown}, 前缀: ${playerPrefix}`, 'green')
  } else if (cachedTown) {
    playerTown = cachedTown.town
  ...
  try {
    ...
    if (tempLogins[code]) {
      ...
      if (isBotReady()) {
        safeChat(`/msg ${username} 网页登录成功，请回到网页继续使用。`, { minGapMs: 1600, dedupeMs: 2500 })
      }
      ...
      if (pendingBindings[code]) {
        ...
        if (user) {
          ...
          if (!isAlreadyBound) {
            ...
            if (isBotReady()) {
              safeChat(`/msg ${username} 绑定成功！你的游戏账号已绑定到仓库系统。`, { minGapMs: 1600, dedupeMs: 2500 })
            }
            
            appendToChatFile(`玩家 ${username} (UUID: ${playerUuid}) 使用验证码 ${code} 成功绑定账号`)
          } else {
            if (isBotReady()) {
              safeChat(`/msg ${username} 该游戏账号已绑定过了。`, { minGapMs: 1600, dedupeMs: 2500 })
            }
          }
        }
      }
...
function createBot() {
  ...
  state.bot = mineflayer.createBot({
    ...
    onMsaCode: (data) => {
      ...
      if (!login.hasLogin) {
        console.log('[LOGIN_ERROR] 未能从 Microsoft 登录响应中提取登录链接或验证码')
        return
      }
      const instruction = formatLoginInstruction(login)
      console.log(`[LOGIN_URL] ${login.url}`)
      console.log(`[LOGIN_CODE] ${login.code || ''}`)
      console.log(`[LOGIN_STEP] ${instruction}`)
      if (process.send) {
        process.send({
          type: 'microsoftLogin',
          login: { url: login.url, code: login.code || '', instruction }
        })
      }
    }
  })
...
process.on('message', async (msg) => {
  if (msg.type === 'cancelScan') {
    ...
    if (!scanAreas || scanAreas.length === 0) {
      if (process.send) process.send({ type: 'scanFailed', error: '没有可用扫描区域' })
      return
    }
    if (process.send) {
    ...
    try {
      ...
      appendToChatFile(`扫描任务失败: ${err.message}`)
      if (process.send) process.send({ type: 'scanFailed', error: err.message })
    } finally {
      if (process.send) process.send({ type: 'task', task: null })
    }
...
async function pollMainBotControlTask() {
  ...
  try {
    ...
    appendToChatFile('拉取主 Bot 控制任务失败: ' + err.message)
  } finally {
...
async function handleMainBotControlTask(task) {
  ...
  if (task.action === 'command') {
    ...
    if (!command || command.length > 256) {
      appendToChatFile('control task skipped: invalid command payload')
      return
    }
    safeChat(command)
    appendToChatFile('control task done: command ' + command)
  }
}
...
async function ensureFoodReady(forceEat = false) {
  ...
  if (foodRecoveryInProgress) return false
  const bot = state.bot
  const food = Number(bot.food ?? 20)
  const carrotCount = getInventoryCount('golden_carrot')
  if (carrotCount < 8) {
    await notifyAdmin('主 Bot 金胡萝卜不足，当前仅剩 ' + carrotCount + ' 个，请补充', 'warning')
    appendToChatFile('金胡萝卜不足，当前仅剩 ' + carrotCount + ' 个，请补充')
  }
  if (!forceEat) return food > 6
  const carrot = bot.inventory.items().find(item => itemNameMatches(item, 'golden_carrot'))
  if (!carrot) return false
  foodRecoveryInProgress = true
  try {
    await bot.equip(carrot, 'hand')
    await bot.consume()
    appendToChatFile('已自动吃金胡萝卜补充饱食度，当前饱食度 ' + Number(bot.food ?? 0))
    return true
  } catch (err) {
    appendToChatFile('自动进食失败: ' + err.message)
    return false
  } finally {
    foodRecoveryInProgress = false
  }
}
...
async function handleTransportTask(task) {
  ...
  try {
    ...
    const movedToHandoff = await moveToPosition(handoffPosition.x, handoffPosition.y, handoffPosition.z, { timeoutMs: 8000, minTimeoutMs: 1200, range: 1.2 })
    if (!movedToHandoff) {
      appendToChatFile('\u4e3b Bot \u672a\u7cbe\u786e\u5230\u8fbe\u4ea4\u63a5\u70b9\uff0c\u6309 2.2.9 \u517c\u5bb9\u6a21\u5f0f\u7ee7\u7eed\u4ece\u5f53\u524d\u4f4d\u7f6e\u8bf7\u6c42\u8fd0\u8f93 Bot \u4f20\u9001')
    }
    await sleep(120)
...
async function waitForTransportAssignment(task) {
  while (isBotReady()) {
    ...
    await sleep(Number(resp && resp.retryAfterMs) || 1000)
  }
...
async function scanSpecifiedChests(scanAreas, options = {}) {
  ...
  for (const area of scanAreas) {
    ...
    if (area.warpCommand) {
      ...
      await safeChat(area.warpCommand, { minGapMs: 1800, dedupeMs: 2000 })
    ...
    if (area.startPositions && area.startPositions.length > 0) {
      for (let i = 0; i < area.startPositions.length; i++) {
        ...
        const moved = await moveToPosition(pos.x, pos.y, pos.z, { timeoutMs: 20000, minTimeoutMs: 5000, range: 1.2 })
        if (!moved) {
          appendToChatFile(`移动到起始位置失败，跳过区域: ${area.name || area.id || i + 1}`)
          try { bot.clearControlStates?.() } catch {}
          skipArea = true
          break
        }
    ...
    appendToChatFile('已到达指定起点，先检索已加载区域内的所有容器坐标，不移动到各个 x 列')
    for (let x = startX; x <= endX; x++) {
      assertScanNotCancelled()
    ...
    const scanOrigin = bot.entity.position.clone()
    chestsToScan.sort((a, b) => {
      const rankDiff = a.rank - b.rank
      if (rankDiff) return rankDiff
      const distanceA = scanOrigin.distanceTo(new vec3(a.x, standY, a.z + 1))
      const distanceB = scanOrigin.distanceTo(new vec3(b.x, standY, b.z + 1))
    ...
    for (const pos of scanList) {
      ...
      try {
        ...
        if (bot.entity.position.distanceTo(standPos) > 2.0) {
          moved = await moveToPosition(x, standY, z + 1, { timeoutMs: 16000, minTimeoutMs: 4000, range: 1.25 })
          if (!moved) {
            appendToChatFile(`移动到箱子旁未完成，改为从当前位置尝试开箱: (${x}, ${y}, ${z})`)
            try { bot.clearControlStates?.() } catch {}
          }
          await new Promise(resolve => setTimeout(resolve, scanMoveDelayMs))
        }
        const opened = await openChestAtPosition(x, y, z, { skipMove: false, standY, maxOpenDistance: 4.8, lookDelay: scanLookDelayMs, closeDelay: scanCloseDelayMs, scanMode, moveTimeoutMs: 16000 })
        if (opened) totalChestsScanned++
      } catch (e) {
