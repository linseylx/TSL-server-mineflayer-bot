...
function emptyTextInfo() {
    return { url: '', code: '', message: '', raw: '', hasUrl: false, hasCode: false, hasLogin: false };
}
...
function extractFromText(text) {
    ...
    const userCode = raw.match(/["']?(?:user_code|userCode|code|otc)["']?\s*:\s*["']?([A-Z0-9-]{4,})["']?/i)?.[1]
        || raw.match(/\[LOGIN_CODE\]\s*([A-Z0-9]{4,}(?:-[A-Z0-9]{2,})?)/i)?.[1]
        || raw.match(/(?:enter|use)\s+the\s+code\s+([A-Z0-9]{4,}(?:-[A-Z0-9]{2,})?)/i)?.[1]
        || raw.match(/(?:\u9a8c\u8bc1\u7801|\u9a57\u8b49\u78bc|\u4ee3\u7801|\u4ee3\u78bc)\s*[:\uFF1A]?\s*([A-Z0-9]{4,}(?:-[A-Z0-9]{2,})?)/i)?.[1]
...
function normalizeMicrosoftLogin(data = {}) {
    ...
    const textInfo = message ? extractFromText(message) : emptyTextInfo();
