...
function getCandidateBaseUrls(mainHost, mainPort, baseUrl = '') {
...
async function requestFromAdmin(mainHost, mainPort, requestPath, options = {}, updateOptions = {}) {
    let lastError = null;
    for (const baseUrl of getCandidateBaseUrls(mainHost, mainPort, updateOptions.baseUrl)) {
        try {
            return await axios.get(`${baseUrl}${requestPath}`, {
                ...options,
                headers: { ...(updateOptions.headers || {}), ...(options.headers || {}) }
            });
        } catch (err) {
            lastError = err;
...
async function checkForUpdate(mainHost, mainPort, updateOptions = {}) {
    try {
        const resp = await requestFromAdmin(mainHost, mainPort, '/api/update/version', { timeout: 5000 }, updateOptions);
        if (resp.data?.success) {
            ...
            const manifestResp = await requestFromAdmin(mainHost, mainPort, '/api/update/local/manifest', { timeout: 5000 }, updateOptions);
...
async function downloadUpdate(mainHost, mainPort, version, updateOptions = {}) {
...
async function performUpdate(mainHost, mainPort, updateOptions = {}) {
    console.log('[更新] 开始检查更新...');
    
    const latestVersion = await checkForUpdate(mainHost, mainPort, updateOptions);
    if (!latestVersion) {
    ...
    const zipPath = await downloadUpdate(mainHost, mainPort, latestVersion, updateOptions);
    if (!zipPath) {
