...
const TRANSPORT_TOKEN_FILE = path.join(__dirname, 'transport_token.txt');
...
function loadOrCreateTransportToken() {
    if (process.env.TRANSPORT_TOKEN) return String(process.env.TRANSPORT_TOKEN).trim();
    try {
        if (fs.existsSync(TRANSPORT_TOKEN_FILE)) {
            const token = fs.readFileSync(TRANSPORT_TOKEN_FILE, 'utf8').trim();
            if (token) {
                try { fs.chmodSync(TRANSPORT_TOKEN_FILE, 0o600); } catch {}
                return token;
            }
        }
    } catch {}
    const token = crypto.randomBytes(32).toString('base64url');
    writeSecretFile(TRANSPORT_TOKEN_FILE, token);
    console.warn('[SECURITY] Generated transport_token.txt. Copy it to local transport config as transportToken.');
    return token;
}
...
const TRANSPORT_TOKEN = loadOrCreateTransportToken();
...
function verifyAnyAuthToken(token) {
    const decoded = decodeAuthToken(token);
    return Boolean(decoded && (decoded.admin || decoded.userId || decoded.username));
}

function verifyAdminAuthToken(token) {
    const decoded = decodeAuthToken(token);
    return Boolean(decoded && decoded.admin === true);
}
...
function requireSocketAuth(socket, next) {
    ...
    if (!token || !verifyAdminAuthToken(decodeURIComponent(token))) {
...
function requireAuth(req, res, next) {
    ...
    if (!token || !verifyAdminAuthToken(token)) {
...
function requireUserAuth(req, res, next) {
    const token = getRequestAuthToken(req);
    if (!token || !verifyAnyAuthToken(token)) {
        return res.status(401).json({ success: false, error: '请先登录' });
    }
    next();
}

function getTransportToken(req) {
    return req.headers['x-transport-token'] || req.query?.transportToken || req.body?.transportToken || '';
}

function verifyTransportToken(token) {
    return safeSecretEqual(token, TRANSPORT_TOKEN);
}
...
function requireTransportAuth(req, res, next) {
    if (!isLocalTransportRequest(req) && !verifyTransportToken(getTransportToken(req))) {
        return res.status(403).json({ success: false, error: 'transport forbidden' });
    }
    next();
}
...
adminApp.get('/api/auth/check', (req, res) => {
    const token = getRequestAuthToken(req);
    const authenticated = Boolean(token && verifyAdminAuthToken(token));
...
adminApp.get('/api/user/info', requireUserAuth, (req, res) => {
    reloadUsersFromDisk();
    const token = getRequestAuthToken(req);
...
adminApp.get('/api/user/bind/list', requireUserAuth, (req, res) => {
    reloadUsersFromDisk();
    const token = getRequestAuthToken(req);
    const user = getUserFromToken(token);
    if (user) {
...
adminApp.post('/api/user/bind/generate', requireUserAuth, (req, res) => {
    reloadUsersFromDisk();
    const token = getRequestAuthToken(req);
    const user = getUserFromToken(token);
...
adminApp.post('/api/user/bind/verify', requireUserAuth, (req, res) => {
...
adminApp.post('/api/user/bind/unbind', requireUserAuth, (req, res) => {
...
adminApp.post('/api/user/change-password', requireUserAuth, (req, res) => {
...
function selectSourceLocationsForQuantity(locations, requestedQuantity) {
    ...
    for (const location of locations) {
        ...
        remaining -= Math.max(0, Number(location.count || 0));
...
function prepareTransportFulfillment(item, requestedQuantity) {
    ...
    if (!isBulkWarehouseItem(item)) {
        const locations = selectSourceLocationsForQuantity(getItemSourceLocations(item), requested);
    ...
    const locations = selectSourceLocationsForQuantity(getItemSourceLocations(item, { storageType }), actualQuantity);
...
adminApp.post('/api/internal/mainbot/control-task', requireAuth, (req, res) => {
    ...
    if (!['eat', 'clearInventory', 'sendMessage', 'command'].includes(action)) {
        return res.json({ success: false, error: '不支持的主 Bot 操作' });
    }
    if (action === 'command') {
        const command = String(req.body?.command || '').trim();
        if (!command || command.length > 256) {
            return res.json({ success: false, error: '命令不能为空且不能超过256字' });
        }
        const task = {
            id: Date.now() + '_' + Math.random().toString(36).slice(2, 8),
            action,
...
adminApp.post('/api/admin/mainbot/command', requireAuth, (req, res) => {
    const command = String(req.body?.command || '').trim();
    if (!command || command.length > 256) {
        return res.json({ success: false, error: '命令不能为空且不能超过256字' });
    }
...
function filterBotLogs(botId, limit = 200) {
    const id = String(botId || 'all').trim();
    const max = Math.max(1, Math.min(500, Number(limit) || 200));
    if (id === 'all') return logs.slice(-max);
    if (id === 'main') {
        return logs.filter(line => line.includes('[主Bot]') || line.includes('[主Bot][') || line.includes('[主Bot控制]') || line.includes('[mainbot-notice]')).slice(-max);
    }
    const escapedId = id.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    const botNumMatch = id.match(/bot(\d+)/i);
    const botNum = botNumMatch ? botNumMatch[1] : '';
    const patterns = [
        new RegExp(`\\[运输Bot\\s+${escapedId}\\]`, 'i'),
        new RegExp(`\\[运输Bot\\s+${escapedId}\\]\\[`, 'i'),
        new RegExp(`\\[TransportBot\\s+${escapedId}\\]`, 'i')
    ];
    if (botNum) {
        patterns.push(new RegExp(`Bot #${botNum}\\b`, 'i'));
        patterns.push(new RegExp(`bot${botNum}\\b`, 'i'));
    }
    return logs.filter(line => patterns.some(pattern => pattern.test(line))).slice(-max);
}

adminApp.get('/api/admin/bot-logs/:id', requireAuth, (req, res) => {
    res.json({ success: true, id: req.params.id, logs: filterBotLogs(req.params.id, req.query.limit) });
});
...
adminApp.post('/api/user/claim', requireUserAuth, (req, res) => {
    handleUserItemRequest(req, res, 'claim');
});

adminApp.post('/api/user/withdraw', requireUserAuth, (req, res) => {
...
require('./routes/transport_failover.js')(adminApp, {
    ...
    addLog, requireAuth, requireTransportAuth, adminIO, shopIO, BOTS_DIR, TRANSPORT_CONFIG_FILE,
...
require('./routes/update_manager.js')(adminApp, {
    addLog, requireAuth, requireTransportAuth, maintenanceState, setSystemStatus, broadcastSystemStatus
});
