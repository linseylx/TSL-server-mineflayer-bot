...
function normalizeMicrosoftLogin(data = {}) {
    ...
    const codeFromText = text.match(/["']?(?:user_code|userCode|code|otc)["']?\s*:\s*["']?([A-Z0-9-]{4,})["']?/i)?.[1]
        || text.match(/\[LOGIN_CODE\]\s*([A-Z0-9]{4,}(?:-[A-Z0-9]{2,})?)/i)?.[1]
        || text.match(/(?:enter|use)\s+the\s+code\s+([A-Z0-9]{4,}(?:-[A-Z0-9]{2,})?)/i)?.[1]
        || text.match(/(?:\u9a8c\u8bc1\u7801|\u9a57\u8b49\u78bc|\u4ee3\u7801|\u4ee3\u78bc)\s*[:\uFF1A]?\s*([A-Z0-9]{4,}(?:-[A-Z0-9]{2,})?)/i)?.[1]
...
function initSocket() {
    ...
    socket.on('mainBotState', (data) => {
        ...
        botFood = data.food || 0;
        if (botStatus === 'online' || botStatus === 'ready') {
            clearMicrosoftLoginModal('main');
            delete consoleState.latestMainLogins['涓籅ot'];
            renderLoginPanel();
        }
        updateMineflayerUI();
    });
    ...
    socket.on('botLog', (data) => {
        ...
        if (data?.type === 'main') appendBotConsoleLine('main', content, 'bot');
        if (data?.type === 'transport' && data.botId) appendBotConsoleLine(data.botId, content, 'bot');
    ...
    socket.on('transportBotLoginUrl', (data) => {
        if (data?.cleared) {
            ...
            clearMicrosoftLoginModal('transport', data.botId);
    ...
    socket.on('mainBotLoginUrl', (data) => {
        ...
        if (botStatus === 'online' || botStatus === 'ready') {
            appendConsoleLine('主Bot 已在线，忽略过期 Microsoft 登录 URL。', 'status');
            return;
        }
        const login = normalizeMicrosoftLogin(data);
...
function clearMicrosoftLoginModal(kind = '', botId = '') {
    const overlay = document.getElementById('modalOverlay');
    const title = document.getElementById('modalTitle')?.textContent || '';
    const body = document.getElementById('modalBody')?.textContent || '';
    if (!overlay?.classList.contains('show')) return;
    if (!/microsoft\.com\/link/i.test(body) && !/Microsoft/i.test(body + title)) return;
    const text = `${title} ${body}`;
    if (kind === 'main' && !/主Bot|涓籅ot|main/i.test(text)) return;
    if (kind === 'transport' && botId && !String(text).includes(String(botId))) return;
    closeModal();
}
    ...
    const content = `
        <div style="padding:20px;height:100%;overflow:auto;">
            <div class="card">
                ...
                <button class="btn btn-primary" onclick="openBotConsole('main', '主Bot', 'main')" style="margin-left:8px">主Bot控制台</button>
                <button class="btn btn-primary" onclick="openTerminal()" style="margin-left:8px">总控制台/登录URL</button>
        ...
        <div style="padding:20px;height:100%;overflow:auto;">
            ...
            <div class="card" style="margin-top:24px;">
                ...
                <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:12px;">
                    ...
                    <div>
                        <label style="display:block;margin-bottom:6px;font-size:13px;color:#64748b;">运输认证 Token</label>
                        <input type="password" class="input-field" id="transportToken" placeholder="粘贴云端 transport_token.txt 内容">
                    </div>
                </div>
...
function loadTransportConfig() {
    fetchWithAuth('/api/admin/transport-config').then(res => res.json()).then(data => {
        ...
        const tokenEl = document.getElementById('transportToken');
        ...
        if (tokenEl) tokenEl.placeholder = config.hasTransportToken ? '已配置，留空则不修改' : '粘贴云端 transport_token.txt 内容';
        if (hintEl) hintEl.textContent = config.mainHost
            ? `当前连接：${config.mainHost}:${config.mainAdminPort || 28474}；Token：${config.hasTransportToken ? '已配置' : '未配置'}`
            : '未设置云端后台地址';
    }).catch(err => {
...
function saveTransportConfig() {
    ...
    const transportToken = document.getElementById('transportToken')?.value.trim();
    if (!mainHost) {
    ...
    fetchWithAuth('/api/admin/transport-config', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ mainHost, mainAdminPort, mainPort, transportToken })
    }).then(res => res.json()).then(data => {
        if (data.success) {
...
function formatHeartbeatAge(ms) {
    const age = Number(ms);
    if (!Number.isFinite(age)) return '无心跳';
    if (age < 1000) return '刚刚';
    if (age < 60000) return `${Math.round(age / 1000)}秒前`;
    return `${Math.round(age / 60000)}分钟前`;
}

function getTransportHealth(bot) {
    const lastSeenAgeMs = Number(bot.lastSeenAgeMs);
    const joinedServer = Boolean((bot.mcOnline || bot.online || bot.connectionState === 'spawned') && bot.ready);
    return {
        joinedServer,
        heartbeatText: formatHeartbeatAge(bot.lastSeenAgeMs)
    };
}

function formatTransportStatus(bot) {
    const health = getTransportHealth(bot);
    if (health.joinedServer) return bot.busy ? '已进服/执行任务' : '已进服/空闲';
    if (bot.running && bot.loginUrl?.url) return '等待 Microsoft 登录';
    if (bot.running) return bot.lastError ? '连接异常，未进服' : '进程运行，等待进服';
...
function formatTransportError(error) {
    ...
    if (/failed to obtain profile data|does the account own minecraft/i.test(text)) {
        return '该微软账号没有可用的 Minecraft Java 版档案。请确认账号已购买正版 Java 版，并点击“清缓存重登”后使用正确账号重新登录。';
    }
    if (/invalid credentials|authentication failed|unauthorized/i.test(text)) {
        return '微软登录凭据已失效，请点击“清缓存重登”，再使用新的登录 URL 和验证码登录。';
    }
    return text;
}
...
function renderBotList() {
    ...
    list.innerHTML = transportBots.map(bot => {
        ...
        const errorText = formatTransportError(bot.lastError);
        const health = getTransportHealth(bot);
        const badgeClass = health.joinedServer ? 'bot-status-running' : (bot.running ? 'bot-status-starting' : ((bot.currentStatus || bot.status) === 'starting' ? 'bot-status-starting' : 'bot-status-stopped'));
        const badgeText = health.joinedServer ? '已进服' : (bot.running ? '连接中' : ((bot.currentStatus || bot.status) === 'starting' ? '启动中' : '已停止'));
        return `
        <div class="bot-card">
            ...
            <div class="bot-details">
                ...
                <div><span class="bot-detail-label">心跳:</span><span class="bot-detail-value">${health.heartbeatText}</span></div>
            ...
            ` : ''}
            ${errorText ? `<div style="margin-top:10px;color:#fca5a5;background:rgba(239,68,68,.08);border:1px solid rgba(239,68,68,.2);border-radius:8px;padding:8px;white-space:pre-wrap;">${escapeHtml(errorText)}</div>` : ''}
            <div class="bot-actions">
                ...
                <button class="bot-action-btn send" onclick="openBotConsole('${escapeAttr(bot.id)}', '运输Bot ${escapeAttr(bot.id)}', 'transport')">独立控制台</button>
                <button class="bot-action-btn send" onclick="openTransportLoginConsole()">登录URL/总控制台</button>
...
function botConsoleWindowId(botId) {
    return 'bot-console-' + String(botId || 'all').replace(/[^a-zA-Z0-9_-]/g, '-');
}

function getBotConsoleOutputId(botId) {
    return botConsoleWindowId(botId) + '-output';
}

function isConsoleNearBottom(output) {
...
function trimConsoleLines(output, maxLines = 320) {
    if (!output) return;
...
function normalizeConsoleContent(content, maxLength = 3000) {
    const text = stripAnsi(content);
    if (text.length <= maxLength) return text;
    return text.slice(0, maxLength) + `\n... 已截断超长日志（原始长度 ${text.length} 字符）`;
}

function appendBotConsoleLine(botId, content, type = 'info') {
    const output = document.getElementById(getBotConsoleOutputId(botId));
    if (!output) return;
    const shouldStickBottom = isConsoleNearBottom(output);
    const time = new Date().toLocaleTimeString('zh-CN');
    output.textContent += `[${time}] ${type.toUpperCase()} ${normalizeConsoleContent(content)}\n`;
    trimConsoleLines(output);
    if (shouldStickBottom) scrollConsoleToBottom(output);
}

function openBotConsole(botId, label = '', kind = 'transport') {
    if (!authToken) { showLoginModal(); return; }
    const safeId = botConsoleWindowId(botId);
    const displayLabel = label || (botId === 'main' ? '主Bot' : `Bot ${botId}`);
    const placeholder = kind === 'main' ? '/say hello 或 /phome xxx' : '/home 或 /say hello';
    const content = `
        <div class="terminal-container">
            <div style="padding:12px;border-bottom:1px solid rgba(255,255,255,.08);background:rgba(15,23,42,.9);">
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;gap:12px;flex-wrap:wrap;">
                    <strong style="color:#e2e8f0;">${escapeHtml(displayLabel)} 独立日志控制台</strong>
                    <div style="display:flex;gap:8px;flex-wrap:wrap;">
                        <button class="btn btn-info btn-sm" onclick="refreshBotConsole('${escapeAttr(botId)}')">刷新日志</button>
                        <button class="btn btn-primary btn-sm" onclick="copyBotConsole('${escapeAttr(botId)}')">复制日志</button>
                        <button class="btn btn-secondary btn-sm" onclick="openTerminal()">打开总控制台</button>
                    </div>
                </div>
                <div style="color:#94a3b8;font-size:12px;">只显示 ${escapeHtml(displayLabel)} 相关日志；下面输入的是 Minecraft 游戏命令，不是服务器 Shell。</div>
            </div>
            <div class="terminal-output" id="${safeId}-output"></div>
            <div class="terminal-input-line">
                <span class="terminal-prompt">${escapeHtml(displayLabel)}</span>:<span class="terminal-path">mc</span>$
                <input type="text" class="terminal-input" id="${safeId}-input" placeholder="${escapeHtml(placeholder)}" onkeypress="if(event.key==='Enter')sendBotConsoleCommand('${escapeAttr(botId)}', '${escapeAttr(kind)}')">
            </div>
        </div>
    `;
    createWindow(safeId, `${displayLabel} 控制台`, 'terminal', content, 860, 620);
    setTimeout(() => {
        refreshBotConsole(botId);
        document.getElementById(`${safeId}-input`)?.focus();
    }, 100);
}

function refreshBotConsole(botId) {
    const output = document.getElementById(getBotConsoleOutputId(botId));
    if (!output) return;
    const shouldStickBottom = isConsoleNearBottom(output);
    ...
    fetchWithAuth(`/api/admin/bot-logs/${encodeURIComponent(botId)}?limit=260`).then(res => res.json()).then(data => {
        if (!data?.success) {
            output.textContent = '加载失败：' + (data?.error || '未知错误') + '\n';
            scrollConsoleToBottom(output);
            return;
        }
        const lines = Array.isArray(data.logs) ? data.logs : [];
        output.textContent = lines.length ? lines.map(line => stripAnsi(line)).slice(-260).join('\n') + '\n' : '暂无该 Bot 日志\n';
        if (shouldStickBottom) scrollConsoleToBottom(output);
    }).catch(err => {
...
function copyBotConsole(botId) {
    const output = document.getElementById(getBotConsoleOutputId(botId));
    copyConsoleText(output?.textContent || '');
}

function sendBotConsoleCommand(botId, kind) {
    const input = document.getElementById(`${botConsoleWindowId(botId)}-input`);
    const command = (input?.value || '').trim();
    if (!command) return;
    input.value = '';
    appendBotConsoleLine(botId, `> ${command}`, 'cmd');
    const url = kind === 'main' || botId === 'main'
        ? '/api/admin/mainbot/command'
        : `/api/admin/transport-bots/${encodeURIComponent(botId)}/command`;
    fetchWithAuth(url, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        ...
        if (data.success) {
            appendBotConsoleLine(botId, '命令已加入队列，等待 Bot 拉取执行', 'status');
            setTimeout(() => refreshBotConsole(botId), 800);
        } else {
            appendBotConsoleLine(botId, '发送失败：' + (data.error || '未知错误'), 'error');
        }
    }).catch(err => appendBotConsoleLine(botId, '发送失败：' + err.message, 'error'));
}
...
async function ccFetch(url, options = {}) {
    ...
    if (url.startsWith('/api/update/') && !url.includes('/status') && !url.includes('/version') && !url.includes('/local/')) {
        let updatePassword = getCcUpdatePassword();
        if (!updatePassword && options.silent) {
            return { success: false, missingUpdatePassword: true, error: 'missing_update_password' };
        }
        if (!updatePassword) updatePassword = requireCcUpdatePassword();
...
async function loadCcBackups() {
    ...
    if (!list) return;
    const updatePassword = getCcUpdatePassword();
    if (!updatePassword) {
        list.innerHTML = `
            <div class="cc-empty">
                <div class="cc-empty-icon">🔒</div>
                <div>输入独立更新密码后加载备份</div>
                <div style="font-size:12px;margin-top:4px;">服务器执行 cat /home/mineflayer/app/update_password.txt 获取</div>
            </div>
        `;
        return;
    }
    ...
    if (!data?.success) {
        list.innerHTML = `
            <div class="cc-empty">
                <div class="cc-empty-icon">⚠️</div>
                <div>${data?.error === 'invalid update password' || data?.error === '更新密码错误' ? '更新密码错误' : '备份加载失败'}</div>
                <div style="font-size:12px;margin-top:4px;">请重新 cat update_password.txt，更新/回滚后密码会轮换</div>
            </div>
        `;
        return;
    }
    ...
    list.innerHTML = '<div class="cc-backup-list">' + data.backups.map(b => `
        <div class="cc-backup-item">
            <div class="cc-backup-info">
                <div class="cc-backup-ver">${escapeHtml(b.version || b.name || '-')}</div>
                <div class="cc-backup-time">${new Date(b.timestamp).toLocaleString('zh-CN')}</div>
            </div>
            <button class="cc-btn cc-btn-danger" style="font-size:12px;padding:6px 12px;" onclick="ccRollback('${escapeAttr(b.name || b.version)}')">回滚</button>
        </div>
    `).join('') + '</div>';
}
