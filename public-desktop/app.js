        ...
        <div id="cc-updates" class="cc-panel">
            <div class="cc-card">
                ...
                <div style="margin-top:14px;display:grid;grid-template-columns:minmax(180px,1fr) auto;gap:10px;align-items:center;">
                    <input id="ccUpdatePassword" type="password" placeholder="独立更新密码：服务器 cat update_password.txt 获取" style="width:100%;padding:10px 12px;border-radius:10px;border:1px solid rgba(255,255,255,.12);background:rgba(15,23,42,.65);color:#e5e7eb;" oninput="saveCcUpdatePassword(this.value)">
...
function requireCcUpdatePassword() {
    ...
    if (!password) {
        showSystemMessage('缺少更新密码', '请先在服务器执行 cat update_password.txt 获取，并在弹窗或更新页输入。', 'error');
        return '';
    }
    return password;
}
